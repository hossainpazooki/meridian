...
def derive(cell):
    """(live_word, twin_word, status_word) for one property, from rows only.

    Vocabulary shared with the vendored conformance pack: CLAIMABLE when the
    live row is GREEN and every twin is RED as planted; PARTIAL when exactly
    one cell kind is present; UNCLAIMED when none, or when the rows ran and
    did not qualify; UNEVALUABLE when the row set cannot be trusted
    (duplicate rows from concurrent writers).
    """
    ...
    if not live or not twins:
        return (live_word, twin_word, "PARTIAL")
    return (live_word, twin_word, "CLAIMABLE" if claimable(cell) else "UNCLAIMED")
...
def render(rows):
    ...
    for p in PROPS:
        live_word, twin_word, status = derive(rows[p])
        n = len(rows[p]["twin"])
        name = PROP_NAMES[p] + (" (%d twins)" % n if n > 1 else "")
        lines.append("| P%d | %s | %s | %s | %s |" % (p, name, live_word, twin_word, status))
