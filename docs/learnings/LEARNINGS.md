...
- [2026-09-03 -- CI measures buf codegen and P7 on Linux](2026-09-03-ci-measures-buf-codegen-and-p7-on-linux.md) -- the tool-directive codegen reproduces the committed .pb.go on ubuntu-24.04 and the gate reads 7/7 there; buf is downloaded and built from source on a cold runner (about 75 s), unpinned beyond go.sum.
- [2026-09-06 -- BASELINE checker refuses MERIDIAN rows](2026-09-06-baseline-checker-refuses-meridian-rows.md) -- "copy, not a translation" holds for the key set and fails at the checker: all 18 rows refused on a PARALLAX-bound `rows` rule and a one-twin-per-lane limit; registration retired by the DATUM ruling.
- [2026-09-09 -- emitter writes the shared row](2026-09-09-emitter-writes-the-shared-row.md) -- kills the two 2026-09-01 schema entries' key names and counts: `schema` added, `gate_sha`/`gate_worktree` replace the project-named keys (17/18 keys); the vendored checker was red for exactly five reasons per row before the change and accepts all 18 after.
